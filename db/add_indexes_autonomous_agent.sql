...
CREATE INDEX IF NOT EXISTS idx_autonomous_agent_config_user_active 
...
CREATE INDEX IF NOT EXISTS idx_autonomous_agent_trades_user_created 
ON autonomous_agent_trades(user_id, created_at DESC);

-- Índice para queries de atualização por user_id
-- Usado em: UPDATE autonomous_agent_config SET ... WHERE user_id = ?
CREATE INDEX IF NOT EXISTS idx_autonomous_agent_config_user_id 
ON autonomous_agent_config(user_id);

-- Índice para queries de trades por user_id
CREATE INDEX IF NOT EXISTS idx_autonomous_agent_trades_user_id 
ON autonomous_agent_trades(user_id);

-- Índice para queries de trades por status e data (usado em estatísticas)
CREATE INDEX IF NOT EXISTS idx_autonomous_agent_trades_status_created 
ON autonomous_agent_trades(status, created_at);

-- Índice para queries de trades por user_id e status
CREATE INDEX IF NOT EXISTS idx_autonomous_agent_trades_user_status 
