...
export class ZeusStrategy implements IAutonomousAgentStrategy, OnModuleInit {
    ...
    private async reconstructStateFromHistory(userId: string): Promise<void> {
        ...
        try {
            // ✅ [ZENIX v3.0] Limitar reconstrução ao início da sessão atual
            // Fetch session_date directly from DB to avoid NodeJS/MySQL timezone translation bugs
            const trades = await this.dataSource.query(
                `SELECT status, profit_loss, created_at 
                 FROM autonomous_agent_trades 
                 WHERE user_id = ?
                   AND strategy = 'zeus' 
                   AND created_at >= (SELECT session_date FROM autonomous_agent_config WHERE user_id = ? LIMIT 1)
                 ORDER BY created_at DESC 
                 LIMIT 100`,
                [userId, userId]
            );
            ...
            if (!trades || trades.length === 0) {
                this.logger.debug(`[Zeus][${userId}] 🔍 Histórico vazio para esta sessão.`);
                state.mode = 'NORMAL';
                state.consecutiveLosses = 0;
                state.profit = config.dailyProfit || 0;
                state.lucroAtual = state.profit;
...
>>>>>>> 9cf299b (feat: zeus agent stop logic and session management fixes)
