...
export class AtlasStrategy implements IStrategy {
  ...
  async activateUser(userId: string, config: any): Promise<void> {
    ...
    // ✅ Determinar símbolo: R_10, R_25, R_100 ou 1HZ10V
    // Default alterado para 1HZ10V conforme recomendação
    let atlasSymbol: 'R_10' | 'R_25' | 'R_100' | '1HZ10V' = '1HZ10V';

    if (symbol && ['R_10', 'R_25', 'R_100', '1HZ10V'].includes(symbol)) {
      atlasSymbol = symbol as 'R_10' | 'R_25' | 'R_100' | '1HZ10V';
      ...
      if (marketLower === 'r_10' || marketLower === 'vol10' || marketLower === 'volatility 10 index') {
        ...
        if (marketLower.includes('vol10') || marketLower.includes('r_10')) {
          atlasSymbol = '1HZ10V'; // ✅ Preferência para 1HZ10V
        }
      }
    }
    ...
    if (isNew || (hasConfigChanges && (now - lastLogTime > 5000))) {
      ...
      const blindadoStatus = stopLossBlindado
        ? (profitTargetNum ? `Ativo (Gatilho: $${(profitTargetNum * 0.40).toFixed(2)})` : 'Ativo (Sem meta!)')
        : 'Inativo';
  ...
  private async executeAtlasOperation(
    ...
    try {
      ...
      // =================================================================================
      // ✅ 1. VERIFICAR STOP PROFIT (Meta de Lucro)
      // =================================================================================
      ...
      // =================================================================================
      // ✅ 2. VERIFICAR STOP-LOSS BLINDADO (Trailing)
      // =================================================================================
      ...
      // Use the profitPeak and stopBlindadoPercent fetched from DB
      const activationThreshold = profitTarget * 0.40;

      if (profitTarget > 0 && profitPeak >= activationThreshold) {
        // MODO BLINDADO: Garante % do lucro máximo atingido
        const factor = stopBlindadoPercent / 100;
  ...
  // Getters
  getTicks(symbol: 'R_10' | 'R_25' | 'R_100' | '1HZ10V'): Tick[] {
