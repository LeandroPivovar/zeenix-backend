...
export class NexusStrategy implements IStrategy {
  ...
  private calculateSMA(period: number): number {
    if (this.ticks.length < period)
      return this.ticks[this.ticks.length - 1]?.value || 0;
  ...
  async activateUser(userId: string, config: any): Promise<void> {
    ...
    this.logger.log(
      `[NEXUS] ${userId} ativado em ${nexusMode} (Input: ${inputMode})`,
    );
    this.saveNexusLog(
      userId,
      'SISTEMA',
  ...
  private async executeOperation(
    ...
    if (stake <= 0) {
      const reason = riskManager.blindadoActive
        ? 'stopped_blindado'
        : 'stopped_loss';
    ...
    try {
      ...
      if (result) {
        ...
        if (status === 'WON') {
          if (wasRecovery) {
            ...
            if (state.vitoriasConsecutivas % 2 === 0) {
              this.saveNexusLog(
                ...
                this.symbol,
                'resultado',
                `🎉 SOROS CICLO PERFEITO! 2 vitórias consecutivas (Nível 1)`,
              );
              this.saveNexusLog(
                state.userId,
                this.symbol,
                'info',
                `Reiniciando para entrada inicial: $${state.apostaInicial.toFixed(2)}`,
              );
          ...
          if (state.vitoriasConsecutivas > 0) {
            this.saveNexusLog(
              ...
              this.symbol,
              'resultado',
              `❌ Soros Nível 1 falhou! Entrando em recuperação`,
            );
          }
        ...
        if (
          state.ultimoLucro > 0 &&
          state.capital - riskManager.getInitialBalance() >=
            riskManager.getProfitTarget()
        ) {
  ...
  private async stopUser(
    state: NexusUserState,
    reason: 'stopped_blindado' | 'stopped_loss' | 'stopped_profit',
  ) {
    const riskManager = this.riskManagers.get(state.userId);
    if (!riskManager) {
    ...
    this.logger.log(
      `[NEXUS] ${state.userId} parado por ${reason}. Status salvo no banco.`,
    );
  }
  ...
  private async executeTradeViaWebSocket(
    ...
    try {
      ...
      this.logger.error(
        `[NEXUS] ❌ Erro ao executar trade via WS: ${error.message}`,
      );
      return null;
    }
  }
      ...
      socket.on('close', () => {
        ...
        if (conn) {
          if (conn.keepAliveInterval)
            clearInterval(conn.keepAliveInterval as any);
