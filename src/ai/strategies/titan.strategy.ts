  ...
  calculateStake(
    ...
    if (this.consecutiveLosses > 0) {
      if (this.riskMode === 'CONSERVADOR') {
        ...
        const targetRecovery = this.totalLossAccumulated * 1.5;
        nextStake = targetRecovery / PAYOUT_RATE;
      }
    } else if (
      this.lastResultWasWin &&
    ...
    if (
      this.useBlindado &&
      profitAccumulatedAtPeak >= activationTrigger &&
      !this._blindadoActive
    ) {
    ...
    if (
      this.useBlindado &&
      !this._blindadoActive &&
      profitAccumulatedAtPeak > 0 &&
      profitAccumulatedAtPeak < activationTrigger
    ) {
      const percentualProgresso =
        (profitAccumulatedAtPeak / activationTrigger) * 100;
      if (userId && symbol && logCallback) {
        logCallback(
          userId,
          symbol,
    ...
    if (potentialBalanceAfterLoss < minAllowedBalance) {
      ...
      if (userId && symbol && logCallback) {
        const balanceRemaining = (currentBalance - minAllowedBalance).toFixed(
          2,
        );
        logCallback(
          userId,
...
export class TitanStrategy implements IStrategy {
  ...
  private check_signal(
    ...
    if (riskManager.consecutiveLosses >= 3) {
      ...
      if (!state.defesaAtivaLogged) {
        this.logger.log(
          `🚨 [TITAN][DEFESA ATIVA] ${riskManager.consecutiveLosses} Losses seguidos. Forçando modo PRECISO.`,
        );
        this.saveTitanLog(
          state.userId,
          this.symbol,
      ...
      if (state.defesaAtivaLogged) {
        this.logger.log(
          `✅ [TITAN][RECUPERAÇÃO] Voltando ao modo ${state.originalMode}.`,
        );
    ...
    const requiredTicks =
      effectiveMode === 'VELOZ' ? 10 : effectiveMode === 'NORMAL' ? 20 : 50;
    ...
    if (effectiveMode === 'VELOZ') {
      ...
      if (signal) {
        const criterio =
          signal === 'PAR'
            ? `Maioria PAR (${evens}/10)`
            : `Maioria ÍMPAR (${10 - evens}/10)`;
        this.saveTitanLog(
          state.userId,
          this.symbol,
          'analise',
  ...
  async activateUser(userId: string, config: any): Promise<void> {
    ...
    const titanMode = (mode || 'VELOZ').toUpperCase();
    ...
    const requiredTicks =
      titanMode === 'VELOZ' ? 10 : titanMode === 'NORMAL' ? 20 : 50;
    this.saveTitanLog(
      userId,
      this.symbol,
      'info',
  ...
  private async executeOperation(
    state: TitanUserState,
    direction: DigitParity,
  ): Promise<void> {
    const riskManager = this.riskManagers.get(state.userId)!;
    const saveTitanLogCallback = (
      userId: string,
      symbol: string,
      type: string,
      message: string,
    ...
    if (stake <= 0) {
      ...
      const sessionStatus = riskManager.blindadoActive
        ? 'stopped_blindado'
        : 'stopped_loss';
      this.tradeEvents.emit({
        ...
        type: sessionStatus,
        strategy: 'titan',
        profitProtected: riskManager.blindadoActive
          ? riskManager.guaranteedProfit
          : undefined,
      });

      await this.deactivateUser(state.userId);
      await this.dataSource.query(
        `UPDATE ai_user_config SET is_active = 0, session_status = ? WHERE user_id = ?`,
        [sessionStatus, state.userId],
      );
      return;
    }
    ...
    try {
      ...
      if (blindadoConfig && blindadoConfig.length > 0) {
        ...
        if (profitTarget > 0 && lucroAtual >= profitTarget) {
          this.logger.log(
            `[TITAN][${state.userId}] 🎯 META DE LUCRO ATINGIDA! Lucro: $${lucroAtual.toFixed(2)} >= Meta: $${profitTarget.toFixed(2)} - BLOQUEANDO OPERAÇÃO`,
          );
        ...
        if (lucroAtual > profitPeak) {
          ...
          this.dataSource
            ...
            .catch((err) =>
              this.logger.error(`[TITAN] Erro ao atualizar profit_peak:`, err),
            );
        }

        // Check if Blindado should activate (40% of profit target reached)
        if (
          config.stop_blindado_percent !== null &&
          config.stop_blindado_percent !== undefined
        ) {
          if (profitPeak >= profitTarget * 0.4) {
            const stopBlindadoPercent =
              parseFloat(config.stop_blindado_percent) || 50.0;
            ...
            // ✅ Log de progresso ANTES de ativar (quando lucro < 40% da meta)
            if (
              !this.blindadoActivatedUsers.has(state.userId) &&
              lucroAtual > 0 &&
              lucroAtual < profitTarget * 0.4
            ) {
              const activationTrigger = profitTarget * 0.4;
              const percentualProgresso =
                (lucroAtual / activationTrigger) * 100;
              this.saveTitanLog(
                state.userId,
                this.symbol,
                'info',
            ...
            if (
              this.blindadoActivatedUsers.has(state.userId) &&
              lucroAtual > (parseFloat(config.profit_peak) || 0)
            ) {
              this.saveTitanLog(
                state.userId,
                this.symbol,
      ...
      this.logger.error(
        `[TITAN][${state.userId}] Erro ao verificar Stop Loss Blindado:`,
        error,
      );
    ...
    if (riskManager.consecutiveLosses > 0 && state.lastDirection !== null) {
      const stakeIndicator =
        riskManager.consecutiveLosses > 1 ? ' - Martingale' : '';
      this.saveTitanLog(
        state.userId,
        this.symbol,
        'sinal',
    ...
    try {
      ...
      if (result) {
        ...
        await this.dataSource
          ...
          .catch((err) =>
            this.logger.error(
              `[TITAN] Erro ao atualizar session_balance:`,
              err,
            ),
          );
        ...
        const exitDigit = result.exitSpot
          ? result.exitSpot.toString().slice(-1)
          : '?';
        ...
        await this.dataSource.query(
          ...
          [status, result.profit, result.exitSpot, tradeId],
        );
          ...
          if (tradeData && tradeData.length > 0) {
            ...
            if (contractId) {
              this.copyTradingService
                ...
                .catch((error: any) => {
                  this.logger.error(
                    `[Titan][CopyTrading] Erro ao atualizar copiadores: ${error.message}`,
                  );
                });
            }
          }
        ...
        this.logger.warn(
          `[TITAN][${state.userId}] Trade ${tradeId} falhou na execução.`,
        );
        await this.dataSource.query(
          `UPDATE ai_trades SET status = 'ERROR' WHERE id = ?`,
          [tradeId],
        );
      }
    } catch (e) {
  ...
  private async createTradeRecord(
    ...
    if (tradeId && this.copyTradingService) {
      this.copyTradingService
        .replicateAIOperation(state.userId, {
          tradeId: tradeId,
          contractId: '',
        ...
        .catch((error) => {
          this.logger.error(
            `[Titan][CopyTrading] Erro ao replicar operação: ${error.message}`,
          );
        });
    }
  ...
  private async executeTradeViaWebSocket(
    ...
    try {
      ...
      this.logger.debug(
        `[TITAN] 📤 [${userId || 'SYSTEM'}] Solicitando proposta | Tipo: ${params.contract_type} | Valor: $${params.amount}`,
      );
      ...
      this.logger.debug(
        `[TITAN] 💰 [${userId || 'SYSTEM'}] Comprando contrato | ProposalId: ${proposalId}`,
      );
      ...
      try {
        ...
        this.logger.error(
          `[TITAN] ❌ Erro ao comprar contrato: ${errorMessage}`,
        );
        if (userId)
          this.saveTitanLog(
            userId,
            this.symbol,
            'erro',
            `❌ Erro ao comprar contrato: ${errorMessage}`,
          );
        return null;
      }
      ...
      if (buyResponse.error) {
        const errorMsg =
          buyResponse.error.message || JSON.stringify(buyResponse.error);
      ...
      this.logger.log(
        `[TITAN] ✅ [${userId || 'SYSTEM'}] Contrato criado | ContractId: ${contractId}`,
      );
      if (userId)
        this.saveTitanLog(
          userId,
      ...
      this.logger.error(
        `[TITAN] ❌ Erro ao executar trade via WS: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private saveTitanLog(
    ...
    this.processTitanLogQueue().catch((error) => {
      this.logger.error(
        `[TITAN][SaveLog] Erro ao processar fila de logs:`,
        error,
      );
    });
  }
  ...
  private async processTitanLogQueue(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `[TITAN][ProcessLogQueue] Erro ao processar logs:`,
        error,
      );
    } finally {
      this.logProcessing = false;
  ...
  private async saveTitanLogsBatch(
    userId: string,
    ...
    try {
      ...
      this.logger.error(
        `[TITAN][SaveLogsBatch][${userId}] Erro ao salvar logs:`,
        error,
      );
    }
  }
